...
export async function createUserFavouritesTable() {
  await pool.query(
    "CREATE TABLE user_favourites(id INT PRIMARY KEY GENERATED ALWAYS AS IDENTITY, user_id INT REFERENCES users(id), xid VARCHAR, title VARCHAR, city VARCHAR, country VARCHAR, suburb VARCHAR, description VARCHAR, image VARCHAR)"
  );
}
...
export async function insertDataUserFavourites() {
  return await pool.query(
    `INSERT INTO user_favourites(user_id, xid, title, city, country, suburb, description, image) (SELECT user_id, xid, title, city, country, suburb, description, image FROM json_populate_recordset(NULL::user_favourites, $1::JSON));`,
